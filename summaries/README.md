...
- **Stage 4** adds WhatsApp as another interface: Meta webhook in, intent routing, then the same services as the REST API; short, personalized Hebrew replies out. 1:1 DMs need no wake word, recent turns are remembered for follow-ups, destructive cancels ask for confirmation first, and a booking with no hour is saved date-only and the bot asks for the time instead of inventing one.
